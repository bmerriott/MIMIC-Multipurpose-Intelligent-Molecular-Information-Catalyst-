...
After installing Ollama, you **must** run this command once to initialize it:

```powershell
ollama serve
```

Wait until you see "Listening on [::]:11434". Then press `Ctrl+C` to stop it.

**Why?** This creates Ollama's configuration files. Mimic AI will auto-start Ollama going forward.
...
**Note:** First model download may take 10-30 minutes depending on your internet speed.
...
1. Ensure you've run `ollama serve` at least once after installing
2. Ensure Ollama is fully closed (Task Manager)
3. Restart Mimic AI
4. Verify at least one model is pulled: `ollama list`
5. Check that no other app is using port 11434
