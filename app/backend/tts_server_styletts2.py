...
def preprocess_text_for_tts(text: str) -> str:
    ...
    # This handles any other Unicode characters we might have missed
    try:
        normalized = unicodedata.normalize('NFKD', text)
        # Encode to ASCII bytes, ignoring non-ASCII characters, then decode back
...
async def synthesize_synthetic(request: SyntheticSynthesizeRequest):
    ...
    try:
        print(f"Received synthetic voice request: {safe_text[:50] if safe_text else 'EMPTY'}...")
        ...
        # Synthesize with synthetic parameters (using preprocessed text)
...
async def synthesize_with_saved_voice(request: SavedVoiceSynthesizeRequest):
    ...
    try:
        print(f"Synthesizing with saved voice '{request.voice_id}': {safe_text[:50]}...")
