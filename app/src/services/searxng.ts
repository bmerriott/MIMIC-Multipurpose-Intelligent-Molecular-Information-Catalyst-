...
class SearXNGService {
  ...
  async checkStatus(): Promise<boolean> {
    // First try direct connection to SearXNG (works in Tauri WebView without CORS)
    try {
      console.log('[SearXNG] Trying direct connection to localhost:8080');
      const directResponse = await fetch('http://localhost:8080/', {
        method: 'GET',
        signal: AbortSignal.timeout(3000)
      });
      if (directResponse.ok) {
        console.log('[SearXNG] Direct connection successful');
        this.available = true;
        return true;
      }
    } catch (e) {
      console.log('[SearXNG] Direct connection failed, trying backend proxy...');
    }
