...
class PersonaRulesManager:
    ...
    def update_rules_from_persona(self, persona_id: str, persona_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Prompt tags section (for AI awareness)
            lines.append("## Prompt Tags")
            lines.append("")
            lines.append("Your prompts may be tagged to indicate the input type:")
            lines.append("- `[TAG: text]` - Standard text input")
            lines.append("- `[TAG: voice]` - Voice/speech input (you are listening)")
            lines.append("- `[TAG: web-search]` - Input with web search results attached")
            lines.append("")
            lines.append("When you see [TAG: voice], you ARE listening and can hear the user.")
            lines.append("When you see [TAG: web-search], current internet data is attached.")
            lines.append("")
            
            # Capabilities
            lines.append("## Capabilities")
            lines.append("")
            lines.append("You operate within the Mimic AI multi-modal system which provides:")
            lines.append("- Access to real-time web search results when provided")
            lines.append("- Ability to read files attached by the user")
            lines.append("- Access to a personal memory folder for long-term recall")
            lines.append("- Voice synthesis for spoken responses")
            lines.append("")
